...
teamhdmUI = {
    ...
    create = function()
        if not teamhdmUI.itemlistElement then
            local itemlistData = teamhdmUI.itemlistData
            ...
            teamhdmUI.itemlistElement = itemlistElement

            local eventHandlers = teamhdmUI.eventHandlers
            ...
            local settings = teamhdmUI.settings

            local keyBinds = teamhdmUI.keyBinds

            bindKey(settings.toggleKey, "down", keyBinds.toggle)

            teamhdmUI.cacheFonts()
        end
    end,

    destroy = function()
        if teamhdmUI.itemlistElement then
            local eventHandlers = teamhdmUI.eventHandlers
            ...
            teamhdmUI.itemlistElement:destroy()

            teamhdmUI.itemlistElement = nil

            teamhdmUI.itemlistData = nil

            teamhdmUI.prepareItemlistData()

            teamhdmUI.elementData = {}
            teamhdmUI.fonts = {}
        end
    end,

    cacheFonts = function()
        local fonts = teamhdmUI.fonts

        fonts["roboto-regular-10"] = silentExport("ep_core", "getFont", "roboto-regular-10")
    end,

    setElementData = function(element, key, data)
        local elementData = teamhdmUI.elementData
    ...
    getElementData = function(element, key)
        ...
        if not data then
            ...
            if data then
                teamhdmUI.setElementData(element, key, data)
            end
        end
    ...
    prepareItemlistData = function()
        if not teamhdmUI.itemlistData then
            local settings = teamhdmUI.settings
            local fonts = teamhdmUI.fonts
            ...
            local itemlistData = {
                ...
                events = {
                    ...
                    onAnimationRender = function(self)
                        ...
                        do
                            local coreElement = teamhdmUI.getElementData(event1.data.arenaElement, "coreElement", false)
                            ...
                            if isElement(coreElement) then
                                ...
                                do
                                    ...
                                    local state = teamhdmUI.getElementData(coreElement, "state", false)
                                    ...
                                    local round = teamhdmUI.getElementData(coreElement, "round", false)
                                ...
                                local teamsCount = teamhdmUI.getElementData(coreElement, "teamsCount", false)
    
                                if teamsCount then
                                    local resultStrings = {}

                                    for i = 1, teamsCount do
                                        local teamElement = teamhdmUI.getElementData(coreElement, "eventTeam" .. i, false)

                                        resultStrings[i] = "#" .. rgbToHex(getTeamColor(teamElement)) .. (teamhdmUI.getElementData(teamElement, "tag", false) or "") .. " #FFFFFF" .. tostring(teamhdmUI.getElementData(teamElement, "points", false) or 0)
                                    end
                ...
                custom = {
                    ...
                    sortPlayers = function(self)
                        ...
                        local sortingFunction = function(a, b)
                            ...
                            local aPoints = teamhdmUI.getElementData(aPlayer, "points", false) or 0
                            local bPoints = teamhdmUI.getElementData(bPlayer, "points", false) or 0
                            ...
                            local playerRenderer = function(self, cell, cellX, cellY, cellSx, cellSy, animationProgress, subPixel)
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        dxDrawText(teamhdmUI.getElementData(player, "points", false) or "-", x, y, x, y, tocolor(255, 255, 255, animationProgress), 1*rel, font, "right", "center", false, false, false, true)
                                    end
                                    ...
                                    do
                                        ...
                                        if isElement(texture) then
                                            ...
                                            dxDrawImage(mathFloor(x), mathFloor(y - imgSize/2), mathFloor(imgSize), mathFloor(imgSize), texture, 0, 0, 0, tocolor(204, 105, 105, animationProgress*(teamhdmUI.getElementData(player, "state", false) == "alive" and 1 or 0.5)))
                                        end
                                    end
                                end
                            end
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        for i, player in pairs(getPlayersInTeam(team)) do
                                            if teamhdmUI.getElementData(player, "state", false) == "alive" then
                                                count = count + 1
                                            end
                                        end
            ...
            teamhdmUI.itemlistData = itemlistData
        end
    end,
    ...
    eventHandlers = {
        ...
        onClientElementDataChange = function(key, oldValue, newValue)
            teamhdmUI.setElementData(source, key, newValue)
        end,

        onClientRestore = function()
            teamhdmUI.itemlistElement.custom.updateRenderTarget(teamhdmUI.itemlistElement)
        end
    },

    keyBinds = {
        toggle = function(key, state)
            teamhdmUI.itemlistElement:setAnimationState(not teamhdmUI.itemlistElement.animation.state)

            --[[ local itemlistElement = teamhdmUI.itemlistElement
...
event1.modes.teamhdm.ui = teamhdmUI
