...
teamwffUI = {
    ...
    create = function()
        if not teamwffUI.itemlistElement then
            local itemlistData = teamwffUI.itemlistData
            ...
            teamwffUI.itemlistElement = itemlistElement

            local eventHandlers = teamwffUI.eventHandlers
            ...
            local settings = teamwffUI.settings

            local keyBinds = teamwffUI.keyBinds

            bindKey(settings.toggleKey, "down", keyBinds.toggle)

            teamwffUI.cacheFonts()
        end
    end,

    destroy = function()
        if teamwffUI.itemlistElement then
            local eventHandlers = teamwffUI.eventHandlers
            ...
            teamwffUI.itemlistElement:destroy()

            teamwffUI.itemlistElement = nil

            teamwffUI.itemlistData = nil

            teamwffUI.prepareItemlistData()

            teamwffUI.elementData = {}
            teamwffUI.fonts = {}
        end
    end,

    cacheFonts = function()
        local fonts = teamwffUI.fonts

        fonts["roboto-regular-10"] = silentExport("ep_core", "getFont", "roboto-regular-10")
    end,

    setElementData = function(element, key, data)
        local elementData = teamwffUI.elementData
    ...
    getElementData = function(element, key)
        ...
        if not data then
            ...
            if data then
                teamwffUI.setElementData(element, key, data)
            end
        end
    ...
    prepareItemlistData = function()
        if not teamwffUI.itemlistData then
            local settings = teamwffUI.settings
            local fonts = teamwffUI.fonts
            ...
            local itemlistData = {
                ...
                events = {
                    ...
                    onAnimationRender = function(self)
                        ...
                        do
                            local coreElement = teamwffUI.getElementData(event1.data.arenaElement, "coreElement", false)
                            ...
                            if isElement(coreElement) then
                                ...
                                do
                                    ...
                                    local state = teamwffUI.getElementData(coreElement, "state", false)
                                    ...
                                    local round = teamwffUI.getElementData(coreElement, "round", false)
                                ...
                                local teamsCount = teamwffUI.getElementData(coreElement, "teamsCount", false)
    
                                if teamsCount then
                                    local resultStrings = {}

                                    for i = 1, teamsCount do
                                        local teamElement = teamwffUI.getElementData(coreElement, "eventTeam" .. i, false)

                                        resultStrings[i] = "#" .. rgbToHex(getTeamColor(teamElement)) .. (teamwffUI.getElementData(teamElement, "tag", false) or "") .. " #FFFFFF" .. tostring(teamwffUI.getElementData(teamElement, "points", false) or 0)
                                    end
                ...
                custom = {
                    ...
                    sortPlayers = function(self)
                        ...
                        local sortingFunction = function(a, b)
                            ...
                            local aPoints = teamwffUI.getElementData(aPlayer, "points", false) or 0
                            local bPoints = teamwffUI.getElementData(bPlayer, "points", false) or 0
                            ...
                            local playerRenderer = function(self, cell, cellX, cellY, cellSx, cellSy, animationProgress, subPixel)
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        dxDrawText(teamwffUI.getElementData(player, "points", false) or "-", x, y, x, y, tocolor(255, 255, 255, animationProgress), 1*rel, font, "right", "center", false, false, false, true)
                                    end
                                    ...
                                    do
                                        ...
                                        if isElement(texture) then
                                            ...
                                            dxDrawImage(mathFloor(x), mathFloor(y - imgSize/2), mathFloor(imgSize), mathFloor(imgSize), texture, 0, 0, 0, tocolor(204, 105, 105, animationProgress*(teamwffUI.getElementData(player, "state", false) == "alive" and 1 or 0.5)))
                                        end
                                    end
                                end
                            end
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        for i, player in pairs(getPlayersInTeam(team)) do
                                            if teamwffUI.getElementData(player, "state", false) == "alive" then
                                                count = count + 1
                                            end
                                        end
            ...
            teamwffUI.itemlistData = itemlistData
        end
    end,
    ...
    eventHandlers = {
        ...
        onClientElementDataChange = function(key, oldValue, newValue)
            teamwffUI.setElementData(source, key, newValue)
        end,

        onClientRestore = function()
            teamwffUI.itemlistElement.custom.updateRenderTarget(teamwffUI.itemlistElement)
        end
    },

    keyBinds = {
        toggle = function(key, state)
            teamwffUI.itemlistElement:setAnimationState(not teamwffUI.itemlistElement.animation.state)

            --[[ local itemlistElement = teamwffUI.itemlistElement
...
event1.modes.teamwff.ui = teamwffUI
