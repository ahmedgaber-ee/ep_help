...
teamwff2UI = {
    ...
    create = function()
        if not teamwff2UI.itemlistElement then
            local itemlistData = teamwff2UI.itemlistData
            ...
            teamwff2UI.itemlistElement = itemlistElement

            local eventHandlers = teamwff2UI.eventHandlers
            ...
            local settings = teamwff2UI.settings

            local keyBinds = teamwff2UI.keyBinds

            bindKey(settings.toggleKey, "down", keyBinds.toggle)

            teamwff2UI.cacheFonts()
        end
    end,

    destroy = function()
        if teamwff2UI.itemlistElement then
            local eventHandlers = teamwff2UI.eventHandlers
            ...
            teamwff2UI.itemlistElement:destroy()

            teamwff2UI.itemlistElement = nil

            teamwff2UI.itemlistData = nil

            teamwff2UI.prepareItemlistData()

            teamwff2UI.elementData = {}
            teamwff2UI.fonts = {}
        end
    end,

    cacheFonts = function()
        local fonts = teamwff2UI.fonts

        fonts["roboto-regular-10"] = silentExport("ep_core", "getFont", "roboto-regular-10")
    end,

    setElementData = function(element, key, data)
        local elementData = teamwff2UI.elementData
    ...
    getElementData = function(element, key)
        ...
        if not data then
            ...
            if data then
                teamwff2UI.setElementData(element, key, data)
            end
        end
    ...
    prepareItemlistData = function()
        if not teamwff2UI.itemlistData then
            local settings = teamwff2UI.settings
            local fonts = teamwff2UI.fonts
            ...
            local itemlistData = {
                ...
                events = {
                    ...
                    onAnimationRender = function(self)
                        ...
                        do
                            local coreElement = teamwff2UI.getElementData(event1.data.arenaElement, "coreElement", false)
                            ...
                            if isElement(coreElement) then
                                ...
                                do
                                    ...
                                    local state = teamwff2UI.getElementData(coreElement, "state", false)
                                    ...
                                    local round = teamwff2UI.getElementData(coreElement, "round", false)
                                ...
                                local teamsCount = teamwff2UI.getElementData(coreElement, "teamsCount", false)
    
                                if teamsCount then
                                    local resultStrings = {}

                                    for i = 1, teamsCount do
                                        local teamElement = teamwff2UI.getElementData(coreElement, "eventTeam" .. i, false)

                                        resultStrings[i] = "#" .. rgbToHex(getTeamColor(teamElement)) .. (teamwff2UI.getElementData(teamElement, "tag", false) or "") .. " #FFFFFF" .. tostring(teamwff2UI.getElementData(teamElement, "sets", false) or 0) .. ":" .. tostring(teamwff2UI.getElementData(teamElement, "points", false) or 0)
                                    end

                                    do
                                        local x = x + sx/2
                                        local y = y + sy + footerSize/2

                                        dxDrawText(tableConcat(resultStrings, ", "), x, y, x, y, tocolor(255, 255, 255, animationProgress), 1*rel, font, "center", "center", false, false, false, true)
                                    end
                                end
                            end
                        end
                ...
                custom = {
                    ...
                    setMaxTeamRows = function(self, maxTeamRows)
                        local custom = self.custom

                        custom.maxTeamRows = maxTeamRows
                    ...
                    sortPlayers = function(self)
                        ...
                        local sortingFunction = function(a, b)
                            ...
                            local aPoints = teamwff2UI.getElementData(aPlayer, "points", false) or 0
                            local bPoints = teamwff2UI.getElementData(bPlayer, "points", false) or 0
                        ...
                        for i = verticalUpLen, 1, -1 do
                            ...
                            if teamElement then
                                ...
                                local nextTeamIndex = custom.findItem2(self, verticalUp, "data", "type", "team", i + 1)
                
                                local teamEndIndex = nextTeamIndex and nextTeamIndex - 1 or verticalUpLen

                                for j = i + 1, teamEndIndex do
                                    data[#data + 1] = verticalUp[j].data
                                end

                                tableSort(data, sortingFunction)

                                for j = 1, #data do
                                    local itemData = data[j]

                                    local index = i + j

                                    verticalUp[index].data, verticalDown[verticalUpLen - index + 1].data = itemData, itemData
                                end
                            end
                        end
                    end,

                    updateMaxTeamRows = function(self)
                        local custom = self.custom

                        local customMaxTeamRows = custom.maxTeamRows
                            ...
                            local playerRenderer = function(self, cell, cellX, cellY, cellSx, cellSy, animationProgress, subPixel)
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        dxDrawText(teamwff2UI.getElementData(player, "points", false) or "-", x, y, x, y, tocolor(255, 255, 255, animationProgress), 1*rel, font, "right", "center", false, false, false, true)
                                    end
                                    ...
                                    do
                                        ...
                                        if isElement(texture) then
                                            ...
                                            dxDrawImage(mathFloor(x), mathFloor(y - imgSize/2), mathFloor(imgSize), mathFloor(imgSize), texture, 0, 0, 0, tocolor(204, 105, 105, animationProgress*(teamwff2UI.getElementData(player, "state", false) == "alive" and 1 or 0.5)))
                                        end
                                    end
                                end
                            end
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        for i, player in pairs(getPlayersInTeam(team)) do
                                            if teamwff2UI.getElementData(player, "state", false) == "alive" then
                                                count = count + 1
                                            end
                                        end
                    ...
                    maxTeamRows = settings.maxTeamRows,
            ...
            teamwff2UI.itemlistData = itemlistData
        end
    end,
    ...
    eventHandlers = {
        ...
        onClientElementDataChange = function(key, oldValue, newValue)
            teamwff2UI.setElementData(source, key, newValue)
        end,

        onClientRestore = function()
            teamwff2UI.itemlistElement.custom.updateRenderTarget(teamwff2UI.itemlistElement)
        end
    },

    keyBinds = {
        toggle = function(key, state)
            teamwff2UI.itemlistElement:setAnimationState(not teamwff2UI.itemlistElement.animation.state)

            --[[ local itemlistElement = teamwff2UI.itemlistElement
...
event1.modes.teamwff2.ui = teamwff2UI
