...
draftUI = {
    ...
    create = function()
        if not draftUI.itemlistElement then
            local itemlistData = draftUI.itemlistData
            ...
            draftUI.itemlistElement = itemlistElement

            local eventHandlers = draftUI.eventHandlers
            ...
            local settings = draftUI.settings

            local keyBinds = draftUI.keyBinds
    ...
    destroy = function()
        if draftUI.itemlistElement then
            local eventHandlers = draftUI.eventHandlers
            ...
            draftUI.itemlistElement:destroy()

            draftUI.itemlistElement = nil

            draftUI.itemlistData = nil

            draftUI.prepareItemlistData()
    ...
    getElementData = function(element, key)
        ...
        if not data then
            ...
            if data then
                draftUI.setElementData(element, key, data)
            end
        end
    ...
    prepareItemlistData = function()
        if not draftUI.itemlistData then
            local settings = draftUI.settings
            local fonts = draftUI.fonts
            ...
            local itemlistData = {
                ...
                events = {
                    ...
                    onAnimationRender = function(self)
                        ...
                        do
                            local coreElement = draftUI.getElementData(event1.data.arenaElement, "coreElement", false)
                            ...
                            if isElement(coreElement) then
                                ...
                                do
                                    ...
                                    local state = draftUI.getElementData(coreElement, "state", false)
                                    ...
                                    local round = draftUI.getElementData(coreElement, "round", false)
                                ...
                                local teamsCount = draftUI.getElementData(coreElement, "teamsCount", false)
    
                                if teamsCount then
                                    local resultStrings = {}

                                    for i = 1, teamsCount do
                                        local teamElement = draftUI.getElementData(coreElement, "eventTeam" .. i, false)

                                        resultStrings[i] = "#" .. rgbToHex(getTeamColor(teamElement)) .. (draftUI.getElementData(teamElement, "tag", false) or "") .. " #FFFFFF" .. tostring(draftUI.getElementData(teamElement, "points", false) or 0)
                                    end
                ...
                custom = {
                    ...
                    sortPlayers = function(self)
                        ...
                        local sortingFunction = function(a, b)
                            ...
                            local aPoints = draftUI.getElementData(aPlayer, "points", false) or 0
                            local bPoints = draftUI.getElementData(bPlayer, "points", false) or 0
                            ...
                            local playerRenderer = function(self, cell, cellX, cellY, cellSx, cellSy, animationProgress, subPixel)
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        dxDrawText((draftUI.getElementData(player, "points", false) or "-") .. " (" .. (draftUI.getElementData(player, "checkpoints", false) or "-") .. ")", x, y, x, y, tocolor(255, 255, 255, animationProgress), 1*rel, font, "right", "center", false, false, false, true)
                                    end
                                    ...
                                    do
                                        ...
                                        if isElement(texture) then
                                            local x = cellX + cellSx - 70*rel

                                            local imgSize = 13*rel

                                            dxDrawImage(mathFloor(x), mathFloor(y - imgSize/2), mathFloor(imgSize), mathFloor(imgSize), texture, 0, 0, 0, tocolor(204, 105, 105, animationProgress*(draftUI.getElementData(player, "state", false) == "alive" and 1 or 0.5)))
                                        end
                                    end
                                end
                            end
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        for i, player in pairs(getPlayersInTeam(team)) do
                                            if draftUI.getElementData(player, "state", false) == "alive" then
                                                count = count + 1
                                            end
                                        end
            ...
            draftUI.itemlistData = itemlistData
        end
    end,
    ...
    eventHandlers = {
        ...
        onClientElementDataChange = function(key, oldValue, newValue)
            draftUI.setElementData(source, key, newValue)
        end,

        onClientRestore = function()
            draftUI.itemlistElement.custom.updateRenderTarget(draftUI.itemlistElement)
        end
    },

    keyBinds = {
        toggle = function(key, state)
            draftUI.itemlistElement:setAnimationState(not draftUI.itemlistElement.animation.state)

            --[[ local itemlistElement = draftUI.itemlistElement
