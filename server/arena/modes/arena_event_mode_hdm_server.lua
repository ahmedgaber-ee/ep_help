...
hdm = {
    settings = {
        ...
        dataFilePath = "server/arena/modes/hdm_data.json",
        resultsFilePath = "server/arena/modes/hdm_results.txt",
        resultsJSONFilePath = "server/arena/modes/hdm_results.json",
    ...
    start = function()
        if not hdm.data then
            ...
            hdm.loadData()
            hdm.updatePlayersOnStart()

            local sourceElement = eventData.sourceElement

            triggerClientResourceEvent(resource, arenaElement, "event1:hdm:onClientEventModeCreatedInternal", sourceElement)

            triggerEvent("event1:hdm:onEventModeCreated", sourceElement)

            outputDebugString(debuggerPrepareString("Event hdm mode created.", 1))
        end
    end,
    ...
    stop = function()
        ...
        if data then
            outputDebugString(debuggerPrepareString("Destroying event1 hdm mode.", 1))
            ...
            triggerEvent("event1:hdm:onEventModeDestroy", sourceElement)

            triggerClientResourceEvent(resource, arenaElement, "event1:hdm:onClientEventModeDestroyInternal", sourceElement)
    ...
    saveData = function()
        local dataFilePath = hdm.settings.dataFilePath
    ...
    saveResults = function()
        ...
        if fileHandler then
            ...
            local pointsOrder = hdm.settings.pointsOrder
    ...
    eventHandlers = {
        onArenaCreated = function()
            ...
            addEventHandler("event1:hdm:onEventStateSet", arenaElement, eventHandlers.onArenaHDMEventStateSet)
        end,
        ...
        onArenaDestroy = function()
            ...
            removeEventHandler("event1:hdm:onEventStateSet", arenaElement, eventHandlers.onArenaHDMEventStateSet)
        ...
        onArenaDeathmatchStateSet = function(state)
            local arenaDeathmatchStateFunction = hdm.arenaDeathmatchStateFunctions[state]
        ...
        onArenaDeathmatchPlayerReachedHunter = function(timePassed)
            ...
            if not isTimer(data.playerHunterTimesTimer) then
                data.playerHunterTimesTimer = setTimer(hdm.timers.hunterTimes, hdm.settings.playerHunterTimesDelay, 1)
            end
            ...
            if getElementData(data.coreElement, "state", false) == "live" then
                ...
                hdm.data.mainClan:setPlayerData(source, "passed", oldPassed + 1)
                hdm.data.mainClan:setPlayerData(source, "points", oldPoints + 1)
                hdm.data.mainClan:setPlayerData(source, "mapHunterReached", true)
            end
        end,
        ...
        onArenaPlayerJoin = function()
            ...
            triggerClientResourceEvent(resource, source, "event1:hdm:onClientEventModeStart", source)
    
            if hasObjectPermissionTo(source, "resource." .. resourceName .. ".mode_wf_referee", false) then
        ...
        onArenaPlayerQuit = function()
            ...
            triggerClientResourceEvent(resource, source, "event1:hdm:onClientEventModeStop", source)
        ...
        onArenaPlayerChangeNick = function(oldNick, newNick)
            hdm.data.mainClan:setPlayerData(source, "nickName", newNick)
        end,

        onArenaPlayerLogin = function()
            if hasObjectPermissionTo(source, "resource." .. resourceName .. ".mode_wff_referee", false) then
                silentExport("ep_core", "setPlayerTeam", source, hdm.data.refereeClan.teamElement)
            end
        end
    },
    ...
    commandHandlers = {
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_wff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if #arg > 0 then
                        ...
                        setElementID(hdm.data.coreElement, eventName)
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if #arg > 0 then
                        ...
                        if not getTeamFromName(teamName) then
                            setTeamName(hdm.data.mainClan.teamElement, teamName)
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_wff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if colorString then
                        ...
                        if r then
                            setTeamColor(hdm.data.mainClan.teamElement, r, g, b)
        ...
        setState = function(player, commandName, state)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_wff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if state then
                        local stateData = hdm.states[state]

                        if stateData then
                            setElementData(hdm.data.coreElement, "state", state)

                            local stateFunction = hdm.stateFunctions[state]
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_wff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if round then
                        ...
                        if round then
                            setElementData(hdm.data.coreElement, "round", round)
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_wff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if totalRounds then
                        ...
                        if totalRounds then
                            setElementData(hdm.data.coreElement, "totalRounds", totalRounds)
                ...
                if playerArenaElement == arenaElement then
                    if playerPartialName then
                        ...
                        if targetPlayer then
                            ...
                            if points then
                                local mainClan = hdm.data.mainClan
        ...
        outputResultsForum = function(player, commandName, forumName, hiddenArg, separatorOffset, ...)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_wff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    local forumID = tonumber(forumName)--hdm.settings.forumNameIDs[forumName]
                    ...
                    if forumID then
                        ...
                        hdm.outputResultsOnForum(forumID, isTopicHidden, tableConcat({unpack(arg, 1, separatorOffset)}, " "), tableConcat({unpack(arg, separatorOffset + 1)}, " "),
    ...
    stateFunctions = {
        ["ended"] = function()
            local resultsString, resultsJSONString = hdm.saveResults()
            ...
            if resultsString then
                ...
                if #refereePlayers > 0 then
                    triggerClientResourceEvent(resource, refereePlayers, "event1:hdm:onClientArenaResultsSaved", eventData.sourceElement, resultsString, resultsJSONString)
                end
            end
        end
    },
...
addEvent("event1:hdm:onEventStateSet")
