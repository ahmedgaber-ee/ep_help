...
draft = {
    settings = {
        ...
        dataFilePath = "server/arena/modes/draft_data.json",
        resultsFilePath = "server/arena/modes/draft_results.txt",
        resultsJSONFilePath = "server/arena/modes/draft_results.json",
    ...
    start = function()
        if not draft.data then
            ...
            local teamsCount = draft.settings.teamsCount
            ...
            addEventHandler("event1:draft:onDraftMarkerHit", arenaElement, eventHandlers.onArenaPlayerDraftMarkerHit)
            ...
            addCommandHandler("sethunterpoints", commandHandlers.setHunterPoints)
            ...
            draft.createEventClans()
            draft.loadData()
            draft.updatePlayersOnStart()

            local sourceElement = eventData.sourceElement

            triggerClientResourceEvent(resource, arenaElement, "event1:draft:onClientEventModeCreatedInternal", sourceElement, teamsCount)

            triggerEvent("event1:draft:onEventModeCreated", sourceElement)
    ...
    stop = function()
        ...
        if data then
            ...
            triggerEvent("event1:draft:onEventModeDestroy", sourceElement)

            triggerClientResourceEvent(resource, arenaElement, "event1:draft:onClientEventModeDestroyInternal", sourceElement)
            ...
            removeEventHandler("event1:draft:onDraftMarkerHit", arenaElement, eventHandlers.onArenaPlayerDraftMarkerHit)
            ...
            removeCommandHandler("sethunterpoints", commandHandlers.setHunterPoints)
            ...
            draft.updatePlayersOnStop()
            draft.saveData()
            draft.destroyEventClans()
    ...
    updatePlayersOnStart = function()
        ...
        local permissionString = "resource." .. resourceName .. ".mode_draft_referee"

        local addEventClansDataSubscriber = draft.addEventClansDataSubscriber
    ...
    saveData = function()
        local dataFilePath = draft.settings.dataFilePath
        ...
        if fileExists(dataFilePath) then
            ...
            if fileHandler then
                ...
                if eventClansData then
                    ...
                    local getEventClanByID = draft.getEventClanByID
            ...
            for i = 1, #eventClans do
                ...
                for serial, data in pairs(clan.serialData) do
                    ...
                    if points and points > 0 then
                        ...
                        playerResult.text = tostring(points) .. " " .. stringRemoveHex(nickNameString) .. " (" .. "passed: " .. tostring(data.passed or 0) .. ")"
    ...
    outputResultsOnForum = function(forumID, isTopicHidden, refereeName, streamerName, callbackFunction)
        ...
        if resultsToJSON then
            ...
            local settings = draft.settings

            local resultsToJSONMapDataLen = #resultsToJSONMapData
            ...
            local maxPlayersStrings = 0

            for i = 1, resultsToJSONMapDataLen do
                maxPlayersStrings = mathMax(maxPlayersStrings, #resultsToJSONMapData[i] - 2)
            end

            for i = 1, maxPlayersStrings do
                htmlStrings[#htmlStrings + 1] = [[
    ...
    addPlayerPoints = function(player, points, addPoints, addCheckpoints)
        local eventClan = draft.getPlayerEventClan(player)

        if eventClan then
            if addPoints then
                eventClan:setPlayerData(player, "points", (getElementData(player, "points", false) or 0) + points)
            end

            if addCheckpoints then
                eventClan:setPlayerData(player, "checkpoints", (getElementData(player, "checkpoints", false) or 0) + points)
            end
            ...
            if addCheckpoints then
                setElementData(teamElement, "checkpoints", (getElementData(teamElement, "checkpoints", false) or 0) + points)
            end
    ...
    canPlayerScorePoints = function(player)
        local eventClan = draft.getPlayerEventClan(player)

        if eventClan then
            local scorers, scorersCount = {}, 0

            for clanPlayer in pairs(eventClan.players) do
                local checkpoints = getElementData(clanPlayer, "checkpoints", false) or 0

                if checkpoints > 0 then
                    scorers[clanPlayer], scorersCount = true, scorersCount + 1

                    if scorersCount > 5 then
                        return false
                    end
                end
            end

            if scorersCount == 5 and not scorers[player] then
                return false
            end
    ...
    getMapCheckpoints = function(mapResourceName)
        local fileName = ":" .. mapResourceName .. "/ep_checkpoints.json"

        if fileExists(fileName) then
            local file = fileOpen(fileName, true)

            if file then
                local checkpoints = fromJSON(fileRead(file, fileGetSize(file))) or {}
    ...
    eventHandlers = {
        onArenaCreated = function()
            ...
            addEventHandler("event1:draft:onEventStateSet", arenaElement, eventHandlers.onArenaWFFEventStateSet)
        end,
        ...
        onArenaDestroy = function()
            ...
            removeEventHandler("event1:draft:onEventStateSet", arenaElement, eventHandlers.onArenaWFFEventStateSet)
        ...
        onArenaDeathmatchStateSet = function(state)
            local arenaDeathmatchStateFunction = draft.arenaDeathmatchStateFunctions[state]
        ...
        onArenaDeathmatchPlayerReachedHunter = function(timePassed)
            ...
            if not isTimer(data.playerHunterTimesTimer) then
                data.playerHunterTimesTimer = setTimer(draft.timers.hunterTimes, draft.settings.playerHunterTimesDelay, 1)
            end

            if getElementData(data.coreElement, "state", false) == "live" and draft.canPlayerScorePoints(source) then
                local eventClan = draft.getPlayerEventClan(source)
                ...
                local hunterPoints = data.hunterPoints or 1

                draft.addPlayerPoints(source, hunterPoints, false, true)

                outputChatBox(getPlayerName(source) .. " #CCCCCChas earned #FFFFFF" .. hunterPoints .. " #CCCCCCpoints for reaching hunter", arenaElement, 255, 255, 255, true)
            end
        end,

        onArenaPlayerDraftMarkerHit = function()
            if source == client then
                local data = draft.data

                if getElementData(data.coreElement, "state", false) == "live" and draft.canPlayerScorePoints(client) then
                    draft.addPlayerPoints(client, 1, false, true)
                end
            end
        end,

        onArenaPlayerJoin = function()
            draft.addEventClansDataSubscriber(source)

            triggerClientResourceEvent(resource, source, "event1:draft:onClientEventModeStart", source, draft.settings.teamsCount)

            local data = draft.data
    
            if hasObjectPermissionTo(source, "resource." .. resourceName .. ".mode_draft_referee", false) then
        ...
        onArenaPlayerQuit = function()
            local eventClan = draft.getPlayerEventClan(source)
            if eventClan then
                local points = getElementData(source, "points", false) or 0
                local checkpoints = getElementData(source, "checkpoints", false) or 0
                local passed = getElementData(source, "passed", false) or 0
                
                -- Save current data WITHOUT adding checkpoints to total points
                eventClan:setPlayerData(source, "points", points)
                eventClan:setPlayerData(source, "checkpoints", checkpoints) -- Keep checkpoints as pending
                eventClan:setPlayerData(source, "passed", passed)
                eventClan:setPlayerData(source, "nickName", getPlayerName(source))
            end
            
            silentExport("ep_core", "setPlayerTeam", source, nil)
            
            triggerClientResourceEvent(resource, source, "event1:draft:onClientEventModeStop", source)
            
            draft.removeEventClansDataSubscriber(source)
        end,
        ...
        onArenaPlayerTeamChange = function(oldTeam, newTeam)
            ...
            if newTeam then
                local eventClan = draft.getEventClanByTeamElement(newTeam)
        ...
        onArenaPlayerLogin = function()
            if hasObjectPermissionTo(source, "resource." .. resourceName .. ".mode_draft_referee", false) then
                silentExport("ep_core", "setPlayerTeam", source, draft.data.refereeClan.teamElement)
            end
        end
    },
    ...
    commandHandlers = {
        join = function(player, commandName, tag)
            ...
            if playerArenaElement == arenaElement then
                if tag then
                    local eventClan = draft.getEventClanByTag(tag)
        ...
        spec = function(player, commandName)
            ...
            if playerArenaElement == arenaElement then
                local spectatorsClan = draft.data.spectatorsClan
        ...
        setEventName = function(player, commandName, ...)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_draft_cmd_" .. commandName, false) then
                local eventData = event1.data
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if #arg > 0 then
                        ...
                        setElementID(draft.data.coreElement, eventName)
        ...
        setState = function(player, commandName, state)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_draft_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if state then
                        local stateData = draft.states[state]

                        if stateData then
                            setElementData(draft.data.coreElement, "state", state)

                            local stateFunction = draft.stateFunctions[state]
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_draft_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if round then
                        ...
                        if round then
                            setElementData(draft.data.coreElement, "round", round)
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_draft_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if totalRounds then
                        ...
                        if totalRounds then
                            setElementData(draft.data.coreElement, "totalRounds", totalRounds)
                ...
                if playerArenaElement == arenaElement then
                    if playerPartialName then
                        ...
                        if targetPlayer then
                            ...
                            if points then
                                local eventClan = draft.getPlayerEventClan(targetPlayer)
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_draft_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if teamsCount then
                        ...
                        draft.settings.teamsCount = teamsCount
        ...
        setHunterPoints = function(player, commandName, points)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_draft_cmd_" .. commandName, false) then
                local eventData = event1.data
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if points then
                        draft.data.hunterPoints = points

                        outputChatBox("#CCCCCCHunter points have been set to #FFFFFF" .. points, player, 255, 255, 255, true)
                    else
                        outputChatBox("#CCCCCCSyntax: #FFFFFF/" .. commandName .. " [points]", player, 255, 255, 255, true)
                    end
                end
            end
        end,
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_draft_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if tag then
                        ...
                        if eventClan then
                            local alivePlayers = draft.getAlivePlayersInClan(eventClan)
        ...
        outputResultsForum = function(player, commandName, forumName, hiddenArg, separatorOffset, ...)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_draft_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    local forumID = tonumber(forumName)--draft.settings.forumNameIDs[forumName]
                    ...
                    if forumID then
                        ...
                        draft.outputResultsOnForum(forumID, isTopicHidden, tableConcat({unpack(arg, 1, separatorOffset)}, " "), tableConcat({unpack(arg, separatorOffset + 1)}, " "),
    ...
    timers = {
        ...
        checkAlivePlayers = function()
            ...
            if getElementData(coreElement, "state", false) == "live" then
                ...
                local getAlivePlayersInClan = draft.getAlivePlayersInClan
    ...
    arenaDeathmatchStateFunctions = {
        ["map loaded"] = function()
            ...
            local getPlayerEventClan = draft.getPlayerEventClan
            ...
            if getElementData(coreElement, "state", false) == "live" then
                ...
                for i = 1, #arenaPlayers do
                    ...
                    if eventClan then
                        eventClan:setPlayerData(arenaPlayer, "checkpoints", 0)
                    end
                end

                for i = 1, #eventClans do
                    setElementData(eventClans[i].teamElement, "checkpoints", 0)
                end
            end

            local mapInfo = getElementData(arenaElement, "mapInfo", false) or {}

            local checkpoints = draft.getMapCheckpoints(mapInfo.resourceName)

            triggerClientResourceEvent(resource, arenaElement, "event1:draft:onClientArenaCheckpointsReceive", eventData.sourceElement, checkpoints)
        ...
        ["running"] = function()
            ...
            --draft.data.checkAlivePlayersTimer = setTimer(draft.timers.checkAlivePlayers, 4000, 0)
        ...
        ["map unloading"] = function()
            ...
            local currentState = getElementData(coreElement, "state", false)
            
            local getPlayerEventClan = draft.getPlayerEventClan
            
            local playerCheckpoints = {}

            if currentState ~= "free" then
                local eventClans = data.eventClans

                for i = 1, #eventClans do
                    local clan = eventClans[i]
                    local serialData = clan.serialData
                    
                    if serialData then
                        for playerSerial, playerData in pairs(serialData) do
                            local checkpoints = playerData.checkpoints or 0
                            
                            if checkpoints > 0 then
                                local currentPoints = playerData.points or 0
                                local newTotalPoints = currentPoints + checkpoints
                                
                                local playerElement = nil

                                for j = 1, #arenaPlayers do
                                    if getPlayerSerial(arenaPlayers[j]) == playerSerial then
                                        playerElement = arenaPlayers[j]
                                ...
                                if playerElement then
                                    clan:setPlayerData(playerElement, "points", newTotalPoints)
                                    clan:setPlayerData(playerElement, "checkpoints", 0)

                                    playerData.points = newTotalPoints
                                    playerData.checkpoints = 0
                                else
                                    playerData.points = newTotalPoints
                                    playerData.checkpoints = 0
                                end
                                ...
                                local teamElement = clan.teamElement
                                local currentTeamPoints = getElementData(teamElement, "points", false) or 0

                                setElementData(teamElement, "points", currentTeamPoints + checkpoints)
                                
                                local playerName = playerData.nickName or "Unknown Player"
                                local r, g, b = getTeamColor(teamElement)
                                local hexColor = "#" .. rgbToHex(r, g, b)
                                local coloredName = hexColor .. playerName
                                
                                playerCheckpoints[#playerCheckpoints + 1] = {
                                    player = nil,
                ...
                for i = 1, #playerCheckpoints do
                    local row = playerCheckpoints[i]

                    local checkpoints = row.checkpoints
                    local playerName = row.coloredName
                    local points = row.totalPoints
                    
                    playersStrings[#playersStrings + 1] = playerName .. " #FFFFFF(" .. checkpoints .. ")"
                    
                    outputChatBox("#CCCCCC" .. i .. nth(i) .. ": " .. playerName .. " #CCCCCChas earned #FFFFFF" .. checkpoints .. " #CCCCCCpoints and now has #FFFFFF" .. points .. " #CCCCCCpoint" .. (points == 1 and "" or "s"), arenaElement, 255, 255, 255, true)
                end
                ...
                for i = 1, #eventClans do
                    ...
                    local teamCheckpoints = getElementData(teamElement, "checkpoints", false) or 0
    ...
    stateFunctions = {
        ["ended"] = function()
            local resultsString, resultsJSONString = draft.saveResults()
            ...
            if resultsString then
                ...
                if #refereePlayers > 0 then
                    triggerClientResourceEvent(resource, refereePlayers, "event1:draft:onClientArenaResultsSaved", eventData.sourceElement, resultsString, resultsJSONString)
                end
            end
        end
    },
...
addEvent("event1:draft:onDraftMarkerHit", true)

addEvent("event1:draft:onEventStateSet")
