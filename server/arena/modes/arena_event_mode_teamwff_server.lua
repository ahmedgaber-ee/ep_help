...
teamwff = {
    settings = {
        ...
        dataFilePath = "server/arena/modes/teamwff_data.json",
        resultsFilePath = "server/arena/modes/teamwff_results.txt",
        resultsJSONFilePath = "server/arena/modes/teamwff_results.json",

        autoPointsOrder = { 10, 8, 6, 4, 3, 2, 1, 1 },
        defaultPointsOrder = { 3, 2, 1, 1 },
        pointsOrder = { 3, 2, 1, 1 },
    ...
    start = function()
        if not teamwff.data then
            ...
            local teamsCount = teamwff.settings.teamsCount
            ...
            teamwff.createEventClans()
            teamwff.loadData()
            teamwff.updatePlayersOnStart()

            local sourceElement = eventData.sourceElement

            triggerClientResourceEvent(resource, arenaElement, "event1:teamwff:onClientEventModeCreatedInternal", sourceElement, teamsCount)

            triggerEvent("event1:teamwff:onEventModeCreated", sourceElement)
    ...
    stop = function()
        ...
        if data then
            ...
            triggerEvent("event1:teamwff:onEventModeDestroy", sourceElement)

            triggerClientResourceEvent(resource, arenaElement, "event1:teamwff:onClientEventModeDestroyInternal", sourceElement)
            ...
            teamwff.updatePlayersOnStop()
            teamwff.saveData()
            teamwff.destroyEventClans()
            ...
            teamwff.settings.pointsOrder = teamwff.settings.defaultPointsOrder
    ...
    updatePlayersOnStart = function()
        ...
        local permissionString = "resource." .. resourceName .. ".mode_teamwff_referee"

        local addEventClansDataSubscriber = teamwff.addEventClansDataSubscriber
    ...
    saveData = function()
        local dataFilePath = teamwff.settings.dataFilePath
        ...
        if fileExists(dataFilePath) then
            ...
            if fileHandler then
                ...
                if eventClansData then
                    ...
                    local getEventClanByID = teamwff.getEventClanByID
    ...
    saveResults = function()
        ...
        if fileHandler then
            ...
            local pointsOrder = teamwff.settings.pointsOrder
    ...
    outputResultsOnForum = function(forumID, isTopicHidden, refereeName, streamerName, callbackFunction)
        ...
        if resultsToJSON then
            ...
            local settings = teamwff.settings
    ...
    addPoints = function(addedBy, players)
        ...
        local getPlayerEventClan = teamwff.getPlayerEventClan
    ...
    eventHandlers = {
        onArenaCreated = function()
            ...
            addEventHandler("event1:teamwff:onEventStateSet", arenaElement, eventHandlers.onArenaWFFEventStateSet)
        end,
        ...
        onArenaDestroy = function()
            ...
            removeEventHandler("event1:teamwff:onEventStateSet", arenaElement, eventHandlers.onArenaWFFEventStateSet)
        ...
        onArenaDeathmatchStateSet = function(state)
            local arenaDeathmatchStateFunction = teamwff.arenaDeathmatchStateFunctions[state]
        ...
        onArenaDeathmatchPlayerReachedHunter = function(timePassed)
            ...
            if not isTimer(data.playerHunterTimesTimer) then
                data.playerHunterTimesTimer = setTimer(teamwff.timers.hunterTimes, teamwff.settings.playerHunterTimesDelay, 1)
            end

            if getElementData(data.coreElement, "state", false) == "live" then
                local eventClan = teamwff.getPlayerEventClan(source)
        ...
        onArenaPlayerJoin = function()
            teamwff.addEventClansDataSubscriber(source)

            triggerClientResourceEvent(resource, source, "event1:teamwff:onClientEventModeStart", source, teamwff.settings.teamsCount)

            local data = teamwff.data
    
            if hasObjectPermissionTo(source, "resource." .. resourceName .. ".mode_teamwff_referee", false) then
        ...
        onArenaPlayerQuit = function()
            ...
            triggerClientResourceEvent(resource, source, "event1:teamwff:onClientEventModeStop", source)
            
            teamwff.removeEventClansDataSubscriber(source)
        end,
        ...
        onArenaPlayerTeamChange = function(oldTeam, newTeam)
            ...
            if newTeam then
                local eventClan = teamwff.getEventClanByTeamElement(newTeam)
        ...
        onArenaPlayerLogin = function()
            if hasObjectPermissionTo(source, "resource." .. resourceName .. ".mode_teamwff_referee", false) then
                silentExport("ep_core", "setPlayerTeam", source, teamwff.data.refereeClan.teamElement)
            end
        end
    },
    ...
    commandHandlers = {
        join = function(player, commandName, tag)
            ...
            if playerArenaElement == arenaElement then
                if tag then
                    local eventClan = teamwff.getEventClanByTag(tag)
        ...
        spec = function(player, commandName)
            ...
            if playerArenaElement == arenaElement then
                local spectatorsClan = teamwff.data.spectatorsClan
        ...
        setEventName = function(player, commandName, ...)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_teamwff_cmd_" .. commandName, false) then
                local eventData = event1.data
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if #arg > 0 then
                        ...
                        setElementID(teamwff.data.coreElement, eventName)
        ...
        setState = function(player, commandName, state)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_teamwff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if state then
                        local stateData = teamwff.states[state]

                        if stateData then
                            setElementData(teamwff.data.coreElement, "state", state)

                            local stateFunction = teamwff.stateFunctions[state]
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_teamwff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if round then
                        ...
                        if round then
                            setElementData(teamwff.data.coreElement, "round", round)
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_teamwff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if totalRounds then
                        ...
                        if totalRounds then
                            setElementData(teamwff.data.coreElement, "totalRounds", totalRounds)
                ...
                if playerArenaElement == arenaElement then
                    if playerPartialName then
                        ...
                        if targetPlayer then
                            ...
                            if points then
                                local eventClan = teamwff.getPlayerEventClan(targetPlayer)
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_teamwff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if teamsCount then
                        ...
                        teamwff.settings.teamsCount = teamsCount
        ...
        toggleAutoPoints = function(player, commandName)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_teamwff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if newState then
                        teamwff.settings.pointsOrder = teamwff.settings.autoPointsOrder
                ...
                if playerArenaElement == arenaElement then
                    ...
                    if #players > 0 then
                        ...
                        if #invalidPlayerPositions == 0 then
                            teamwff.addPoints(player, players)
                        else
            ...
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_teamwff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    if tag then
                        ...
                        if eventClan then
                            local alivePlayers = teamwff.getAlivePlayersInClan(eventClan)
        ...
        outputResultsForum = function(player, commandName, forumName, hiddenArg, separatorOffset, ...)
            if hasObjectPermissionTo(player, "resource." .. resourceName .. ".mode_teamwff_cmd_" .. commandName, false) then
                ...
                if playerArenaElement == arenaElement then
                    local forumID = tonumber(forumName)--teamwff.settings.forumNameIDs[forumName]
                    ...
                    if forumID then
                        ...
                        teamwff.outputResultsOnForum(forumID, isTopicHidden, tableConcat({unpack(arg, 1, separatorOffset)}, " "), tableConcat({unpack(arg, separatorOffset + 1)}, " "),
    ...
    timers = {
        ...
        checkAlivePlayers = function()
            ...
            if getElementData(coreElement, "state", false) == "live" then
                ...
                local getAlivePlayersInClan = teamwff.getAlivePlayersInClan
    ...
    arenaDeathmatchStateFunctions = {
        ...
        ["running"] = function()
            ...
            --teamwff.data.checkAlivePlayersTimer = setTimer(teamwff.timers.checkAlivePlayers, 4000, 0)
        end,
        ...
        ["map unloading"] = function()
            ...
            if data.autoPointsState and getElementData(data.coreElement, "state", false) == "live" then
                ...
                if #playerNames > 0 then
                    teamwff.addPoints(nil, playerNames)
                end
            end
    ...
    stateFunctions = {
        ["ended"] = function()
            local resultsString, resultsJSONString = teamwff.saveResults()
            ...
            if resultsString then
                ...
                if #refereePlayers > 0 then
                    triggerClientResourceEvent(resource, refereePlayers, "event1:teamwff:onClientArenaResultsSaved", eventData.sourceElement, resultsString, resultsJSONString)
                end
            end
        end
    },
...
addEvent("event1:teamwff:onEventStateSet")
